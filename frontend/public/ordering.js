...
export function createOrdering({ onHome }) {
  ...
  function orderView() {
    return `<div class="view-heading"><div><h1>${t('yourOrder')}</h1><p>${t('reviewHint')}</p></div></div>${!order.lines.length?`<div class="empty-order"><span class="empty-icon">${svg('cart')}</span><h2>${t('empty')}</h2><p>${t('emptyHint')}</p><button class="primary-button" data-tab="menu">${t('browse')}${svg('arrow')}</button></div>`:`${order.confirmed?`<div class="confirmation-banner" role="status">${svg('check')}<div><strong>${t('confirmed')}</strong><p>${t('confirmedHint')} ${order.lines.some(line=>line.kitchen_status==='received')?t('cancelReceived'):''}</p></div></div>`:''}<div class="order-lines">${order.lines.map(line=>{const item=order.item(line.id);const canCancel=line.sent&&line.kitchen_status==='received';return `<article class="order-line" aria-label="${esc(name(item))}">${picture(item,true)}<div class="line-details"><h2>${esc(name(item))}</h2>${optionsText(line.options)?`<p>${esc(optionsText(line.options))}</p>`:''}<strong class="line-price">${money(line.subtotal_cents)}</strong>${line.sent?`<p>${t('confirmedLine')}</p>`:''}<div class="line-bottom"><div class="quantity-control" role="group" aria-label="${t('quantity')} ${esc(name(item))}"><button data-quantity="${esc(line.key)}" data-delta="-1" aria-label="${t('minus')} ${esc(name(item))}" ${line.quantity<=1||line.sent?'disabled':''}>${svg('minus')}</button><span aria-label="${t('quantity')}: ${line.quantity}">${line.quantity}</span><button data-quantity="${esc(line.key)}" data-delta="1" aria-label="${t('plus')} ${esc(name(item))}" ${line.quantity>=20||line.sent?'disabled':''}>${svg('plus')}</button></div><div class="line-edit-buttons"><button data-edit="${esc(line.key)}" ${line.sent?'disabled':''}>${svg('edit')}${t('change')}</button><button data-remove="${esc(line.key)}" ${line.sent&&!canCancel?'disabled':''}>${svg('trash')}${t(canCancel?'cancelItem':'remove')}</button></div></div></div></article>`;}).join('')}</div><div class="order-total"><span>${t('total')}</span><strong>${money(order.total)}</strong></div><div class="order-final-actions">${listenButton(orderSummary(),'order-summary',t('readOrder'),'read-order')}${order.confirmed?`<button class="primary-button" data-action="new-order">${t('newOrder')}</button>`:`<button class="primary-button" data-action="confirm">${svg('check')}${t('confirm')}</button>`}<p class="demo-caption">${t('notSent')}</p></div>`}`;
  }
  function chatView() {
  ...
  root.addEventListener('click',async event=>{
    ...
    if(button.dataset.add){editLine(null,order.item(button.dataset.add).id);return;}
