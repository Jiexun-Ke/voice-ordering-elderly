...
- **Order:** server-backed quantities, preference changes, removals and totals. Confirmation includes dine-in/takeaway and reaches the backend's **mock kitchen**. It does not submit to a real restaurant or charge money. Confirmed lines cannot be edited, but a line may be cancelled while the kitchen status is still `received`; cancellation is blocked once preparation starts.
